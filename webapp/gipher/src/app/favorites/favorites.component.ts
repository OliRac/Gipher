...
export class FavoritesComponent implements OnInit {
  ...
  iconPath: string = 'assets/img/favorite-sm.png';
  ...
  ngOnInit(): void {
    if (this.sharedFavoritesList.isEmpty) {
      this.favoriteService
        .getAllFavorites(this.userService.getUserSession())
        .subscribe((data) => {
          this.sharedFavoritesList.favorites = parseTenorResponseForGifs(
            data,
            true
          );
        });
  ...
  markAsFavorite(gifUrl: string) {
    this.favoriteService
      .checkifFavorite(this.userService.getUserSession(), gifUrl)
      .subscribe((data) => {
        this.favorites = data;
      });
  }
