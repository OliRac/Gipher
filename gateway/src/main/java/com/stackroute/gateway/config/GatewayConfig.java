...
public class GatewayConfig {
    ...
    public RouteLocator myRoutes(RouteLocatorBuilder builder) {
        return builder.routes().route(  r -> r.path("/auth/**")
                                ...
                                .route( r -> r.path("/api/v1/recommendation-service/**")
                                        .filters(f -> f.filter(filter)).uri("http://recommendedservice:9095"))
                                .build();
        //testing
//        return builder.routes().route(  r -> r.path("/auth/**")
...
//                .route( r -> r.path("/api/v1/recommendation-service/**")
//                        .filters(f -> f.filter(filter)).uri("http://localhost:9095"))
//                .build();
    }
}